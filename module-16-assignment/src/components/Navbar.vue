<template>
  <nav id="navbar"
       :class="{ '-translate-y-full absolute': !showNavbar }" class="block w-full sticky top-0 z-10 transition-all duration-300">
    <!-- Navbar Body -->
    <div
      ...
      <router-link to="/#landing-page">
        <div
          ...
          <div v-if="!portfolio.greeting.logo.custom">
            <img id="logo-img"
                 :src="getImageUrl(portfolio.greeting.logo.link)" class="scale-75 md:scale-100 origin-left drop-shadow-lg"/>
          </div>
          <!-- Custom Logo -->
          <div v-else>
            <div
              ...
              <p>&lt;</p>
              <p id="custom-logo" class="text-5xl md:text-7xl">S.H.L</p>
              <p>/></p>
            </div>
          </div>
        </div>
      ...
      <button :class="['inline-flex items-center p-2 ml-3 text-sm text-gray-500 lg:hidden transition-all motion-reduce:transition-none duration-500', showTransition ? 'opacity-1 blur-0' : 'opacity-0 blur-sm']"
      ...
      <div id="navbar-default" class="hidden w-full lg:block lg:w-auto items-center">
        <ul class="font-medium flex flex-col p-4 lg:p-0 mt-4 lg:flex-row lg:space-x-8 lg:mt-0">
          ...
          <li :class="['transition-all motion-reduce:transition-none duration-500 delay-[100ms]', showTransition ? 'translate-y-0 opacity-1' : '-translate-y-4 opacity-0']">
            <router-link class="block py-2 pl-3 pr-4 hover:text-link-color" to="/#experience-section">Experience
            </router-link>
          </li>
    ...
    <NavTransition>
      <div v-show="dropDownActive"
           class="block w-full absolute lg:hidden px-9 pb-4 bg-white/[.9] dark:bg-slate-900/[.8] backdrop-blur-sm">
        <ul>
          <li>
            <router-link class="block py-5 pl-3 hover:text-link-color" to="/#about-section">😎 About</router-link>
          </li>
          <li>
            <router-link class="block py-5 pl-3 hover:text-link-color" to="/#experience-section">🛡️ Experience
            </router-link>
          </li>
          <li>
