<template>
  <div
    ...
    <div :class="project.alignLeft ? 'text-left' : 'lg:text-right'" class="flex flex-col space-y-4">
      ...
      <div :class="!project.alignLeft ? 'lg:justify-end' : ''" class="flex space-x-6 items-center text-slate-400">
        <a v-for="link in project.links" :href="link.url"
           ...
           target="_blank">
                    <span v-if="link.type == 'git'" class="flex items-center space-x-2">
                        <font-awesome-icon class="h-7 w-7" icon="fa-brands fa-github"></font-awesome-icon>
