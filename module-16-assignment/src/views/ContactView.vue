<template>
  <section id="contact-section" ref="contactSection" class="min-h-screen w-full flex flex-col">
    <div :class="[visible ? 'translate-y-0 opacity-1 blur-0' : 'translate-y-4 opacity-0 blur-sm', 'transition-all motion-reduce:transition-none duration-500']"
    ...
    <div
        :class="['flex flex-col my-auto transition-all motion-reduce:transition-none duration-500 delay-300', visible ? 'translate-y-0 opacity-1 blur-0' : 'translate-y-4 opacity-0 blur-sm']">
      <div v-if="content.formEmbedLink">
        <iframe :src="content.formEmbedLink" class="mb-10 min-h-screen w-full" frameborder="0" marginheight="0"
        ...
        <div class="flex flex-col sm:flex-row items-center justify-center space-y-5 sm:space-x-5">
          <template v-if="content.externalLink.link.email">
            ...
            <p class="text-sm mt-8 sm:mt-0 text-slate-500 dark:text-slate-300">
              {{ content.externalLink.responseTimeMessage }}</p>
          </template>
          <template v-else>
